...
def main():
    ...
    train_loader = DataLoader(TimeIndexDataset(X_train, Y_train), batch_size=TRAIN_BATCH_SIZE, shuffle=True,
                              num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=True)
    val_loader = DataLoader(TimeIndexDataset(X_val, Y_val), batch_size=TEST_BATCH_SIZE, shuffle=False,
                            num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=False)
    test_loader = DataLoader(TimeIndexDataset(X_test, Y_test), batch_size=TEST_BATCH_SIZE, shuffle=False,
                             num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY, drop_last=False)
    ...
    model = DiffusionTransformer(
        n_tokens=N, in_dim=in_dim, d_model=96, n_head=4, n_layers=3, d_ff=256, dropout=0.1
    ).to(DEVICE)
    ...
    print(f"[Test] rmse={test_rmse:.6f} mae={test_mae:.6f} corr={test_corr:.4f} var={test_var:.4f}")
