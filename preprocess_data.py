...
def add_features_and_labels(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    ...
    df["vol_diff_ma_5"] = df[VOLUME_COL].diff().rolling(window=5, min_periods=3).mean()
    df["vol_diff_ma_20"] = df[VOLUME_COL].diff().rolling(window=20, min_periods=6).mean()
    df["vol_diff_ma_60"] = df[VOLUME_COL].diff().rolling(window=60, min_periods=20).mean()
    ...
    # 다음날 없는 마지막 행 제거
    df = df.dropna(subset=[LABEL_COL], how="any")

    return df


def clip_features_before_split(df: pd.DataFrame) -> pd.DataFrame:
...
def build_dataset():
    ...
    for csv_path in tqdm(csv_paths, desc="Reading CSVs"):
        ...
        # 원본 컬럼 drop
        df = df.drop(columns=[DATE_COL, OPEN_COL, CLOSE_COL, HIGH_COL, LOW_COL], errors="ignore")

        # (요청) all_dfs에 넣기 전에: label, 거래량 관련 제외하고 -10~10 클리핑
        df = clip_features_before_split(df)
    ...
    for ticker in tqdm(train_dfs.keys(), desc="Standardize & Save tensors"):
        # 각각 label clip 일관 적용
        train_dfs[ticker][LABEL_COL] = np.clip(train_dfs[ticker][LABEL_COL].values, -3.0, 3.0)
        val_dfs[ticker][LABEL_COL] = np.clip(val_dfs[ticker][LABEL_COL].values, -3.0, 3.0)
        test_dfs[ticker][LABEL_COL] = np.clip(test_dfs[ticker][LABEL_COL].values, -3.0, 3.0)
